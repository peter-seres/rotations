...
class Quaternion(np.ndarray):
    ...
    def imag(self, value: Vector) -> None:
        ...
        if len(value) != 3:
            raise ValueError(
                "Setting imaginary part must be done using an array with 3 elements."
            )
...
class UnitQuaternion(Quaternion):
    ...
    def R_bi(self) -> RotationMatrix:
        ...
        R = (
            np.eye(3)
            + 2 * self.real * so3.hat(self.imag)
            + 2 * np.linalg.matrix_power(so3.hat(self.imag), 2)
        )
    ...
    def from_rotmat(R: Union[RotationMatrix, array_like]):
        ...
        if type(R) not in [RotationMatrix, *array_like]:
            raise TypeError(
                "Input matrix must be of type RotationmMatrix or array-like (numpy array, list, tuple)"
            )
        ...
        return UnitQuaternion.from_parts(
            real=real_part, imag=so3.vex(so3.asymproj(R)) / (2.0 * real_part)
        )
    ...
    def from_euler_angles(
        ...
        q_yaw = UnitQuaternion.from_parts(
            real=np.cos(yaw / 2.0), imag=np.array([0.0, 0.0, np.sin(yaw / 2.0)])
        )

        q_pitch = UnitQuaternion.from_parts(
            real=np.cos(pitch / 2.0), imag=np.array([0.0, np.sin(pitch / 2.0), 0.0])
        )

        q_roll = UnitQuaternion.from_parts(
            real=np.cos(roll / 2.0), imag=np.array([np.sin(roll / 2.0), 0.0, 0.0])
        )
